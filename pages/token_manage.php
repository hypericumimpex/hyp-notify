<div class="wrap">
   ...
   <h2><?php echo __('Manage Subscribers', 'smpush-plugin-lang')?>
   ...
   <a href="javascript:" onclick="smpush_open_service(-1,2)" class="add-new-h2"><?php echo __('Add New Subscriber', 'smpush-plugin-lang')?></a><img src="<?php echo smpush_imgpath.'/wpspin_light.gif';?>" alt="" class="smpush_service_-1_loading" style="display:none" />
   </h2>
      ...
      <input type="hidden" name="noheader" value="1" id="smpush-noheader-value" />
         <div class="col-wrap">
          <p class="search-box smpush-canhide">
              <label class="screen-reader-text"><?php echo __('Search Subscribers:', 'smpush-plugin-lang')?></label>
              <input type="search" name="query" value="<?php echo (!empty($_GET['query']))?$_GET['query']:'';?>">
              <input type="submit" id="search-submit" class="button" value="<?php echo __('Search Subscribers', 'smpush-plugin-lang')?>">
           </p>
          <div class="tablenav top">
            ...
            <div class="alignleft actions smpush-canhide">
              <select name="device_type">
                ...
                <option value="<?php echo $types_name->iosfcm_name;?>" <?php if($_GET['device_type'] == $types_name->iosfcm_name){?>selected="selected"<?php }?>>iOS FCM</option>
                ...
                <option value="<?php echo $types_name->opera_name;?>" <?php if($_GET['device_type'] == $types_name->opera_name){?>selected="selected"<?php }?>>Opera</option>
                <option value="<?php echo $types_name->edge_name;?>" <?php if($_GET['device_type'] == $types_name->edge_name){?>selected="selected"<?php }?>>Edge</option>
                <option value="<?php echo $types_name->samsung_name;?>" <?php if($_GET['device_type'] == $types_name->samsung_name){?>selected="selected"<?php }?>>Samsung Browser</option>
                <option value="<?php echo $types_name->fbmsn_name;?>" <?php if($_GET['device_type'] == $types_name->fbmsn_name){?>selected="selected"<?php }?>>Facebook Messenger</option>
                <option value="<?php echo $types_name->fbnotify_name;?>" <?php if($_GET['device_type'] == $types_name->fbnotify_name){?>selected="selected"<?php }?>>Facebook Notification</option>
                <option value="<?php echo $types_name->email_name;?>" <?php if($_GET['device_type'] == $types_name->email_name){?>selected="selected"<?php }?>>Newsletter</option>
              </select>
              <select name="status">
             ...
             <table class="wp-list-table widefat fixed tags" cellspacing="0" <?php if(get_bloginfo('version') < 3.8){?>style="table-layout: auto"<?php }?>>
                <thead>
                   <tr>
                      ...
                      <th scope="col" class="manage-column smpush-canhide"><span><?php echo __('Subscriber Token', 'smpush-plugin-lang')?></span></th>
                      <th scope="col" class="manage-column smpush-center"><span><?php echo __('Platform', 'smpush-plugin-lang')?></span></th>
                      <th scope="col" class="manage-column smpush-center"><span><?php echo __('User', 'smpush-plugin-lang')?></span></th>
                      <th scope="col" class="manage-column smpush-center"><span><?php echo __('Location', 'smpush-plugin-lang')?></span></th>
                      <th scope="col" class="manage-column smpush-canhide"><span><?php echo __('Information', 'smpush-plugin-lang')?></span></th>
                      <th scope="col" class="manage-column smpush-canhide"><span><?php echo __('Time', 'smpush-plugin-lang')?></span></th>
                ...
                <?php if($tokens){$counter = 0;foreach($tokens AS $token){$counter++;?>
                   <tr id="smpush-service-tab-<?php echo $token->id;?>" class="smpush-service-tab <?php if($counter%2 == 0){echo 'alternate';}?>">
                      ...
                      <td class="name column-name smpush-canhide"><span><?php echo gmdate(self::$wpdateformat, strtotime($token->timepost))?></span></td>
