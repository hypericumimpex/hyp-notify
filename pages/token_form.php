   ...
   <div id="post-body" class="metabox-holder columns-2">
      <div id="post-body-content" class="edit-form-section">
         <div id="namediv" class="stuffbox">
            <h3><label><?php echo ($token['id']==0)? __('Add New Subscriber', 'smpush-plugin-lang') : __('Edit Subscriber Info', 'smpush-plugin-lang') ;?></label></h3>
            <div class="inside">
                ...
                <tbody>
                  ...
                  <tr valign="top">
                     ...
                     <select name="device_type">
                        ...
                        <option value="<?php echo $types_name->iosfcm_name;?>" <?php if($token['device_type'] == $types_name->iosfcm_name){?>selected="selected"<?php }?>>iOS FCM</option>
                        ...
                        <option value="<?php echo $types_name->opera_name;?>" <?php if($token['device_type'] == $types_name->opera_name){?>selected="selected"<?php }?>>Opera</option>
                        <option value="<?php echo $types_name->edge_name;?>" <?php if($token['device_type'] == $types_name->edge_name){?>selected="selected"<?php }?>>Edge</option>
                        <option value="<?php echo $types_name->samsung_name;?>" <?php if($token['device_type'] == $types_name->samsung_name){?>selected="selected"<?php }?>>Samsung Browser</option>
                        <option value="<?php echo $types_name->fbmsn_name;?>" <?php if($token['device_type'] == $types_name->fbmsn_name){?>selected="selected"<?php }?>>Facebook Messenger</option>
                        <option value="<?php echo $types_name->fbnotify_name;?>" <?php if($token['device_type'] == $types_name->fbnotify_name){?>selected="selected"<?php }?>>Facebook Notification</option>
                        <option value="<?php echo $types_name->email_name;?>" <?php if($token['device_type'] == $types_name->email_name){?>selected="selected"<?php }?>>Newsletter</option>
                     </select>
                     </td>
                  </tr>
                  <tr valign="top">
